...
impl<K: 'static + AsRef<[u8]>, V: AsHashTree + 'static> Node<K, V> {
    ...
    fn left_hash_tree(&self) -> HashTree {
    ...
    fn right_hash_tree(&self) -> HashTree {
    ...
    fn data_tree(&self) -> HashTree {
        labeled(self.key.as_ref(), self.value.as_hash_tree())
    }

    fn subtree_with<'a>(&'a self, f: impl FnOnce(&'a V) -> HashTree) -> HashTree {
        labeled(self.key.as_ref(), f(&self.value))
    }

    fn witness_tree(&self) -> HashTree {
        labeled(self.key.as_ref(), pruned(self.value.root_hash()))
    }

    fn full_witness_tree<'a>(n: &'a NodeRef<K, V>, f: fn(&'a Node<K, V>) -> HashTree) -> HashTree {
        match n {
            None => empty(),
...
fn three_way_fork(l: HashTree, m: HashTree, r: HashTree) -> HashTree {
    match (l.root, m.root, r.root) {
        ...
        (HashTreeNode::Empty(), m, r) => fork(HashTree { root: m }, HashTree { root: r }),
        (HashTreeNode::Pruned(lhash), HashTreeNode::Pruned(mhash), HashTreeNode::Pruned(rhash)) => {
            pruned(fork_hash(&lhash, &fork_hash(&mhash, &rhash)))
        }
        (l, HashTreeNode::Pruned(mhash), HashTreeNode::Pruned(rhash)) => {
            fork(HashTree { root: l }, pruned(fork_hash(&mhash, &rhash)))
        }
        (l, m, r) => fork(
            HashTree { root: l },
            fork(HashTree { root: m }, HashTree { root: r }),
        ),
    }
}
