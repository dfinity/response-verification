...
The [HttpCertificationTree] can be easily initialized with the [Default] trait and entries can be added to, removed from, have witnesses generated by, the tree using the [HttpCertificationTreeEntry] struct. The [HttpCertificationTreeEntry] requires an [HttpCertification] and an [HttpCertificationPath]. For example:
