...
```shell
icp deploy http_certification_custom_assets_backend
```
...
```shell
echo "http://$(icp canister status http_certification_custom_assets_backend --id-only).localhost:8000"
```

Alternatively, to make a request with `curl`:

```shell
curl "http://$(icp canister status http_certification_custom_assets_backend --id-only).localhost:8000" --resolve "$(icp canister status http_certification_custom_assets_backend --id-only).localhost:8000:127.0.0.1"
```
